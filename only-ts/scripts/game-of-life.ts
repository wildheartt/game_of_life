import {
  GameOfLifeLocalStorage,
  GameOfLifeNodes,
  GameOfLifeParams,
  GameOfLifeWorkerResult,
...
export class GameOfLife {
  ...
  private sendWorkerInitFields({
    ...
    rows: number;
    cols: number;
  }): Promise<GameOfLifeWorkerResult> {
    this.load();
    this.disableInputs();
    ...
    const resultWorker = new Promise<GameOfLifeWorkerResult>(
      (resolve, reject) => {
  ...
  private initEventListeners(): void {
    ...
    this.nodes.popupGenerateNode.addEventListener(
      'click',
    ...
    document.addEventListener('pointerup', (event) => {
      if (event.button === 2 || event.button === 1) return false; // Если это правая или средняя кнопка мыши, это не тот клик
      const clickOnPopupBool = this.isClickOnElement(
        event,
        this.nodes.popupNode
      );
      if (clickOnPopupBool === true) return;
      if (this.isRealDragging === false && this.loading === false)
    ...
    this.nodes.popupSpeedRangeNode.addEventListener(
      'input',
    ...
    this.nodes.popupInputRowsNode.addEventListener('input', () => {
      const newRows = restrictInputValue(
        this.nodes.popupInputRowsNode,
        0,
        2000
      );
    ...
    this.nodes.popupInputColsNode.addEventListener('input', () => {
      const newCols = restrictInputValue(
        this.nodes.popupInputColsNode,
        0,
        2000
      );
  ...
  private togglePopup = (popupHidden: boolean): void => {
    if (this.localStorageUse === true)
      this.setLocalStorage('popupHidden', this.popupHidden);
  ...
  private sendWorkerResizeField(
    newRows: number,
    newCols: number,
    random: boolean
  ): Promise<void | GameOfLifeWorkerResult> {
    this.load();
    this.disableInputs();
  ...
  private sendWorkerDeleteOrCreateCell(
    typeAction: 'delete' | 'create',
    key: string
  ): Promise<GameOfLifeWorkerResult> {
    this.load();
  ...
  private initNodes(injectedNode: HTMLElement): void {
    ...
    const canvasNode = this._querySelector<HTMLCanvasElement>(
      injectedNode,
      '.canvas'
    );
    const wrapperCanvasNode = this._querySelector<HTMLElement>(
      injectedNode,
      '.wrapper-canvas'
    );
    const popupPlayNode = this._querySelector<HTMLElement>(
      popupNode,
      '.popup__play'
    );
    const popupPauseNode = this._querySelector<HTMLElement>(
      popupNode,
      '.popup__pause'
    );
    const popupTimeNode = this._querySelector<HTMLElement>(
      popupNode,
      '.popup__time'
    );
    const popupPopulationNode = this._querySelector<HTMLElement>(
      popupNode,
      '.popup__population'
    );
    const popupCyclesNode = this._querySelector<HTMLElement>(
      popupNode,
      '.popup__cycles'
    );
    const popupGenerateNode = this._querySelector<HTMLElement>(
      popupNode,
      '.popup__generate'
    );
    const popupRandomCheckboxNode = this._querySelector<HTMLInputElement>(
      popupNode,
      '.popup__random-checkbox'
    );
    const popupSpeedRangeNode = this._querySelector<HTMLInputElement>(
      popupNode,
      '.popup__speed-range'
    );
    const popupSpeedInfoNode = this._querySelector<HTMLElement>(
      popupNode,
      '.popup__speed-info'
    );
    const popupClearNode = this._querySelector<HTMLElement>(
      popupNode,
      '.popup__clear'
    );
    const popupStepNode = this._querySelector<HTMLElement>(
      popupNode,
      '.popup__step'
    );
    const popupLoadNode = this._querySelector<HTMLElement>(
      popupNode,
      '.popup__load'
    );
    const popupInputRowsNode = this._querySelector<HTMLInputElement>(
      popupNode,
      '.popup__rows'
    );
    const popupInputColsNode = this._querySelector<HTMLInputElement>(
      popupNode,
      '.popup__cols'
    );
    const popupCloseNode = this._querySelector<HTMLInputElement>(
      popupNode,
      '.popup__close'
    );
  ...
  private _querySelector<T = Element>(
  ...
  private setColorCell = (): void => {
    this.colorCell = getComputedStyle(
  ...
  private drawField(): void {
    ...
    this.ctx.clearRect(
      0,
      0,
      this.nodes.canvasNode.width,
      this.nodes.canvasNode.height
    );
  ...
  private startGame(): void {
    ...
    this.animationObj = requestInterval(
      this.stepGame,
      this.interval / this.speed ** 1.5
    );
